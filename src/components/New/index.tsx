...
import {
    ...
    TEST_KEY,
    RECOMMENDATION_KEY
} from '../../pages/SingleDirection/constants'
...
const New: FC = () => {
    ...
    const renderTabs = () =>
        tabsCatalogConfig.map((options) => {
            ...
            return (
                <Component key={id} onClick={() => setActiveTab(key)} fullWidth sx={{backgroundColor, color}}
                           variant='contained'>
                    {title}
                </Component>
            )
    ...
    const renderMenuList = () => {
        ...
        const menuLists = {
            ...
            [TEST_KEY]: (
                <MenuList<TestType> queryParams={{from, to}} activeTab={activeTab} request={TestService.getTestByDate}
